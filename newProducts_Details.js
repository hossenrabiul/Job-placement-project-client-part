async function fetchPostDetails() {
    ...
    try {
        const response = await fetch(`https://sporting-server-xi.vercel.app/posts/newpostdetail/${postId}/`);
        ...
        div.innerHTML = `
            ...
            <div class="p-4">
                ...
                <button class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-700 add-to-cart" data-id="${post.id}" data-image = "${post.image}"  data-name="${post.name}" data-price="${post.price}" data-stock="${post.storkQuantity}">
                Add to Cart
                </button>
            </div>
        `;
